...
public final class AWTFontRenderer {
    ...
    private static final float IMG_SIZE = 1024;
    ...
    public AWTFontRenderer(Font awtFont, boolean fractionalMetrics) {
    ...
    private static void drawQuad(float x, float y, float width, float height, float x1, float y1, float width1, float height1) {
        float x2 = x1 / IMG_SIZE;
        float y2 = y1 / IMG_SIZE;
        float width2 = width1 / IMG_SIZE;
        float height2 = height1 / IMG_SIZE;

        GL11.glTexCoord2f(x2 + width2, y2);
        GL11.glVertex2d(x + width, y);
        GL11.glTexCoord2f(x2, y2);
        GL11.glVertex2d(x, y);
        GL11.glTexCoord2f(x2, y2 + height2);
        GL11.glVertex2d(x, y + height);
        GL11.glTexCoord2f(x2, y2 + height2);
        GL11.glVertex2d(x, y + height);
        GL11.glTexCoord2f(x2 + width2, y2 + height2);
    ...
    private DynamicTexture setupTexture(Font font, CharData[] chars) {
        return new DynamicTexture(generateFontImage(font, chars));
    }
    ...
    private void setupTextures() {
        this.texturePlain = setupTexture(awtFont, charData);
        this.textureBold = setupTexture(awtFont.deriveFont(Font.BOLD), boldChars);
        this.textureItalic = setupTexture(awtFont.deriveFont(Font.ITALIC), italicChars);
        this.textureItalicBold = setupTexture(awtFont.deriveFont(Font.BOLD | Font.ITALIC), boldItalicChars);
    }
        ...
        for (int i = 0, size = text.length(); i < size; i++) {
            ...
            if (character == '\u00A7' && i + 1 < size) {
                ...
                if (colorIndex < 16) {
                    ...
                    GlStateManager.color((colorCode >> 16 & 0xFF) / 255.0F, (colorCode >> 8 & 0xFF) / 255.0F, (colorCode & 0xFF) / 255.0F, 255);
                ...
                if (strikethrough) {
                    drawLine(x, y + charData[character].height / 2.0F, x + charData[character].width - 8.0D, y + charData[character].height / 2.0F, 1.0F);
                }

                if (underline) {
                    drawLine(x, y + charData[character].height - 2.0D, x + charData[character].width - 8.0D, y + charData[character].height - 2.0D, 1.0F);
                }
